export const SITE = {
  title: "Tiago Martins Pinto - Art, Technology, Education",
  description: "Portfolio of Tiago Martins Pinto, artist, teacher, researcher, and creative technologist.",
  ogTitle: "Tiago Martins Pinto - Art, Technology, Education",
  ogDescription: "Artist, teacher, researcher, and creative technologist.",
  socialImage: "assets/projects/eating-together/eating-together-01.jpeg",
  socialImageAlt: "Eating Together workshop table with participants gathered around collective activity",
  canonicalUrl: "https://www.tiagomartinspinto.com/",
  intro: "",
  header: {
    name: "TIAGO MARTINS PINTO",
    mark: ":-]",
    contactLabel: "Message me!",
    contactEmail: "tiago.martinspinto@aalto.fi"
  },
  footer: {
    socialLinks: [
      {
        label: "GitHub",
        url: "https://github.com/tiagomartinspinto"
      },
      {
        label: "LinkedIn",
        url: "https://www.linkedin.com/in/ptiagomp/"
      },
      {
        label: "CV",
        url: "https://docs.google.com/document/d/1f442zMAeaa5GwRuYdz0S69q2sy36VBsB?rtpof=true&usp=drive_fs"
      },
      {
        label: "ORCID",
        url: "https://orcid.org/0000-0002-0731-8420"
      }
    ],
    aboutTitle: "About Me",
    aboutLines: [
      "Art, code, learning,",
      "and material practice.",
      "One of my earliest memories",
      "is the smell of the ocean",
      "and the wind inside",
      "a striped beach canvas tent."
    ],
    location: "Based in Helsinki, Finland.",
    roleLinks: [
      {
        label: "Researcher @ AALTO",
        url: "https://research.aalto.fi/en/persons/tiago-martins-pinto"
      },
      {
        label: "Course Instructor @ AALTO",
        url: "https://tiagomartinspinto.github.io/aalto-programming-visual-artists/"
      },
      {
        label: "Workshop Master @ AALTO",
        url: "https://www.aalto.fi/en/management-of-school-of-arts-design-and-architecture/arts-ict-group"
      }
    ]
  }
};
